...
@section('content')
    <form action="/transaksi/{{ $transaksi->id }}" method="POST">
        ...
        <div class="form-group p-3">
            ...
            <select name="id_pembeli" class="form-control" required>
                ...
                @foreach ($pembeli as $item)
                    <option value="{{ $item->id }}" {{ $item->id == $transaksi->id_pembeli ? 'selected' : '' }}>{{ $item->nama_pembeli }}</option>
                @endforeach
            </select>
            @error('id_pembeli')
        ...
        <div class="form-group p-3">
            ...
            <select name="id_kasir" class="form-control" required>
                ...
                @foreach ($kasir as $item)
                    <option value="{{ $item->id }}" {{ $item->id == $transaksi->id_kasir ? 'selected' : '' }}>{{ $item->nama_kasir }}</option>
                @endforeach
            </select>
            @error('id_kasir')
                <div class="alert alert-danger">{{ $message }}</div>
        ...
        <div class="form-group p-3">
            ...
            <input type="text" name="pembayaran" class="form-control" value="{{ $transaksi->pembayaran }}" required>
            @error('pembayaran')
                <div class="alert alert-danger">{{ $message }}</div>
            @enderror
        </div>
